...
By default, if this property is not specified, dawson expects to find public assets in the `process.cwd() + /assets/` folder; if such folder does not exist, dawson skips the assets upload step.  
If you specify a value for this property, such folder must exist when dawson starts.  
Specify `false` to skip uploading assets.  
