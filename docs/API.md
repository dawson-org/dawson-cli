...
* **isEventHandler** (boolean, defaults to `false`): set to `true` if this function will be used as an event handler (S3 Notifications, DynamoDB Streams, SNS etc.). This makes sense only if `path === false`.
